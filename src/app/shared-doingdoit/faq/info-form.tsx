...
const addUploadedImage = async (imageNumber: number, url: string) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      ///const res = await fetch(`/api/doingdoit/faq/getAllCategories?_limit=1000&_page=1`);
      // POST
  ...
  useEffect(() => {
    ...
    const fetcher = async () => {
      ...
      const res = await fetch(`/api/doingdoit/faq/getOne?id=${id}`);
  ...
  const onSubmit: SubmitHandler<BoardFormTypes> = (data) => {
    ...
    const fetcher = async () => {
      const res = await fetch(`/api/doingdoit/faq/updateOne`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
