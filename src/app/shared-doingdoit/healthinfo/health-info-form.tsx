...
import FormGroup from '@/app/shared-doingdoit/form-group-large';
...
const removeFeedImage = async (imageNumber: number) => {
  ...
  useEffect(() => {
    ...
    const fetcher = async () => {
      ...
      // TypeError: Cannot read properties of undefined (reading 'smarteditor')
      // bug fix
      // https://stackoverflow.com/questions/66035829/reactjs-cannot-read-properties-of-undefined-reading-smarteditor



      ///(window as any).oEditors.getById["smarteditor"].setIR(data?.data?.content);
  ...
  */
  ...
  const onSubmit: SubmitHandler<BoardFormTypes> = (data) => {
    ...
    const fetcher = async () => {
      const res = await fetch(`/api/doingdoit/healthinfo/updateOne`, {
        method: 'POST',
        headers: {
          ...
          <>
            ...
            <div className="mb-10 grid divide-y divide-solid divide-gray-200 border rounded-lg ">
              ...
              */}
              ...
              {/*
              <div className="mb-10 w-full flex flex-col items-start justify-center">
                <Text  className="p-5 font-medium text-gray-700 dark:text-gray-200 ">
                  내용
                </Text>
