...
const FrameComponentFeeds: NextPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const res = await fetch(`/api/doingdoit/feed/getAllByEmailScrap?_email=${email}&_limit=30&_page=1&_q=${searchTerm}`);
      ...
      const res1 = await fetch(`/api/doingdoit/feed/getAllByEmailScrapCount?_email=${session?.user?.email}&_q=${searchTerm}`);
