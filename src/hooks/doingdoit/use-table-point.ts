...
export function useTable<T extends AnyObject>(
  ...
  */
  ...
  console.log("useTable-point searchTerm:", searchTerm);
  ...
  const fetchData = async (
    ...
    const res = await fetch('/api/doingdoit/point/getAll', {
      method: 'POST',
      headers: {
  ...
  //,[ userId, searchTerm, sortConfig, currentPage, countPerPage, pointTypeArray, startDate, endDate,]);

  , [ userId, sortConfig, currentPage, countPerPage, ]);



 

  // if searchTerm, regTypeArray, startDate, endDate is changed, go to first page

  useEffect(() => {
      
      setCurrentPage(1);
  
  } , [isFiltered, searchTerm, pointTypeArray, startDate, endDate, ]);
