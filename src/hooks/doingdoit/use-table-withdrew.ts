...
export function useTable<T extends AnyObject>(
  ...
  */
  ...
  function handleDelete(id: string | string[]) {
    ...
    */
    ...
    const deleteData = async () => {
      ...
      await fetch(`/api/doingdoit/user/deleteOne?id=${id}`);
    
    
      const res = await fetch('/api/doingdoit/user/getAllWithdrewers', {
        method: 'POST',
        headers: {
      ...
      console.log("posts.data.users=======",  posts?.data?.users);
      */
  ...
  const fetchData = async (
    ...
    //const res = await fetch(`/api/doingdoit/user/getAllUsers?_limit=${countPerPage}&_page=${currentPage}&_sort=${sortConfig.key}&_order=${sortConfig.direction}&_q=${searchTerm}`);

    // api /api/doingdoit/user/getAllUsers post
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const res = await fetch(`/api/doingdoit/user/getAllWithdrewers?_limit=${countPerPage}&_page=${currentPage}&_sort=${sortConfig.key}&_order=${sortConfig.direction}&_q=${searchTerm}`);
    ...
    const fetchTotalCount = async () => {
      const res = await fetch(`/api/doingdoit/user/getWithdrewerCount?_q=${searchTerm}`);
  ...
  ,[ searchTerm, currentPage, sortConfig, filters, totalCount, countPerPage]);
