...
export async function getAll({
  ...
  if (endDate === '') {
    ...
    try {
      ...
      const query = `
      SELECT 
      healthinfos.sequenceNumber,
      healthinfos.id, healthinfos.userId, healthinfos.title, healthinfos.content, healthinfos.images, healthinfos.tags, healthinfos.isTop, healthinfos.createdAt, healthinfos.updatedAt, healthinfos.viewCount,
      users.email AS userEmail, users.nickname AS userNickname, users.avatar AS userAvatar
      FROM healthinfos
      LEFT JOIN users ON healthinfos.userId = users.id
      WHERE
      healthinfos.createdAt >= ? AND healthinfos.createdAt <= ?

      AND (healthinfos.title LIKE ?  OR JSON_EXTRACT(healthinfos.tags, '$') LIKE ?)

      ORDER BY healthinfos.isTop DESC, healthinfos.createdAt DESC
      LIMIT ?, ?
      `;
...
export async function updateOne(
  ...
  try {
      ...
      // update sequenceNumber of healthinfos table order by isTop DESC, createdAt DESC

      const query2 = `
      SELECT id FROM healthinfos
      ...
      for (let i = 0; i < rowsBoard.length; i++) {
        ...
        const query = `
        UPDATE healthinfos SET sequenceNumber = ? WHERE id = ?
        `;
        const values = [
           rowsBoard.length - i,
          rowsBoard[i].id
...
export async function registerOne (
  ...
  // insert healthinfos table
  const connection = await connect();

  try {

    const query = `
    INSERT INTO healthinfos 
...
*/
 ...
 export async function like(
  ...
  if (results) {
    ...
    const likeCollection = client.db('doingdoit').collection('healthinfo_likes');
